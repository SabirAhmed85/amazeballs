                    ...
                    if event.phase == "began" then
                        ...
                        local function centerManualFan ()
                            ...
                            and event.target.enabled == true then
                                ...
                                instance2:setSequence(direction)
                                Runtime:removeEventListener("enterFrame", centerManualFan)
                            end
                        end
                        Runtime:addEventListener("enterFrame", centerManualFan)
                ...
                and ballActivateButtonSet == true then
                    ...
                    elseif event.phase == "moved" then
                        ...
                        if (ySwipe) < 10 and (ySwipe) > -10 and (eventTotalTime) > 10 and (eventTotalTime) < 600 then
                            if (xSwipe) > 7 and spitterCounter == 0 and event.target.rightEnabled == true and shouldBallMoveState == "true" then
                                direction = "right"
                                thisBallSpitter = event.target
                                moveBallFromSpitter()
                            elseif (xSwipe) < -7 and spitterCounter == 0 and event.target.leftEnabled == true and shouldBallMoveState == "true" then
                                direction = "left"
                                thisBallSpitter = event.target
                                moveBallFromSpitter()
                            end
                        ...
                        if (xSwipe) < 10 and (xSwipe) > -10 and (eventTotalTime) > 10 and (eventTotalTime) < 600 then
                            if (ySwipe) > 7 and spitterCounter == 0 and event.target.downEnabled == true and shouldBallMoveState == "true" then
                                direction = "down"
                                thisBallSpitter = event.target
                                moveBallFromSpitter()
                            elseif (ySwipe) < -7 and spitterCounter == 0 and event.target.upEnabled == true and shouldBallMoveState == "true" then
                                direction = "up"
                                thisBallSpitter = event.target
                                moveBallFromSpitter()
                            end
    ...
    if event.target.name == "play" then
  	if event.phase == "began" then
        ...
        elseif event.phase == "moved" then
            event.target.isFocus = true
            display.getCurrentStage():setFocus( event.target )
            if event.x > event.target.x + event.target.contentWidth/2
            or event.y < event.target.y - event.target.contentHeight/2 then
                if event.target.isFocus == true then
                    backBtn:setSequence("default")
...
function scene:createScene( event )
    ...
    --table.insert(transitionArrayIndex,{{"triangle"},{"flip-vertical"}})
    
    switch = display.newSprite(switchImageSheet, switchSequenceData)
    table.insert(shapeArray, switch)
    table.insert(shapeArrayParameters,{"switch","switch1", 1, 1, 1, 3, "flip-vertical", "triangle", "two-way"})
            ...
            or shapeArrayParameters[z][7] == "flip-vertical" then
                for a = 1, #shapeArrayParameters do
                    if shapeArrayParameters[a][2] == shapeArrayParameters[z][8] then
                        ...
                        shapeArray[a].switchValueText = display.newImage("flipPositionSwitchNumber"..numberOfSwitchesCounter..".png")
                        frontScreenObjectsGroup:insert( shapeArray[a].switchValueText )
                        shapeArray[a].switchValueText:toFront()
                        shapeArray[a].switchValueText.x = shapeArray[a].x
                        shapeArray[a].switchValueText.y = shapeArray[a].y
                        ...
                        for b =1, #switchValueTextPositioningArray do
                            if shapeArrayParameters[a][7] == switchValueTextPositioningArray[b][1] then
                                if thisFlipTransitionDirection == "flip-horizontal" then
                                    shapeArray[a].switchValueText.x  = shapeArray[a].switchValueText.x + switchValueTextPositioningArray[b][2]
                                    shapeArray[a].switchValueText.y  = shapeArray[a].switchValueText.y + switchValueTextPositioningArray[b][3]
                                elseif thisFlipTransitionDirection == "flip-vertical" then
                                    shapeArray[a].switchValueText.x  = shapeArray[a].switchValueText.x + switchValueTextPositioningArray[b][4]
                                    shapeArray[a].switchValueText.y  = shapeArray[a].switchValueText.y + switchValueTextPositioningArray[b][5]
                                end
                            end
                        end
                    end
