...
function new()
	...
	function transitions:cancel(pausing, ignorePaused)
		...
		if self.db[id].paused and (not ignorePaused or ignorePaused == nil) then
			self.db[id].paused 		= false
			self.db[id].pausedSince 	= 0

		else		
			transition.cancel(self.db[id].transition)
			self.db[id].transition = nil
		end
		...
		if pausing == nil or pausing == false then
			...
			self.db[id].goingOn 		= 0
			self.db[id].transitionId 	= 1
	...
	function transitions:pause(id)

		if self.db[id].paused then
			return false
		end

		self.db[id]:cancel(true)
		--self.db[id].paused = true
		--self.db[id].pausedSince = system.getTimer()
	end

	function transitions:resume(id)

		if not self.db[id].paused then
			return false
		end

		local timeNow = system.getTimer()

		self.db[id].params.time = self.db[id].params.time - (self.db[id].pausedSince - self.db[id].timeStarted)
		self.db[id].timeStarted = timeNow
		self.db[id].transition = transition.to(self.db[id].object, self.db[id].params)
