local t = {}
local addToShapeArray = require("utils.add-to-shape-array");
local addToTransitionArray = require("utils.add-to-transition-array");

local createLevelObjects = function (mainFunc, shapeArrayParameters)
    local newShape = function(type, name, location, subType, props)
        addToShapeArray(shapeArrayParameters, type, name, location, subType, props);
    end
    local newTransition = function(shapeName, transitionType, props, positionArray)
        addToTransitionArray(mainFunc.allLevelSettings.transitionArrayIndex, shapeArrayParameters, shapeName, transitionType, props, positionArray);
    end

    --SCREEN 1-1
    newShape("shape","triangle1_1_1", {1, 1, 2, 1}, "triangleBottomRightShape", {1})
    newShape("manualFan","manualFan1_1_1", {1, 1, 5, 1}, "left")
    newShape("autoFan","autoFan1_1_1", {1, 1, 7, 1}, "left")
    newShape("manualFan","manualFan2_1_1", {1, 1, 5, 2}, "down")
    newShape("shape","triangle1_1_1", {1, 1, 7, 3}, "triangleTopRightShape", {1, 1})
    newShape("autoFan","autoFan2_1_1", {1, 1, 7, 4}, "up")
    newTransition("autoFan2_1_1", "slide", {"orange"}, {{1,1,7,4},{1,1,7,5}})
    newShape("door","door1_1_1", {1, 1, 2, 5}, "down")
    newShape("manualFan","manualFan3_1_1", {1, 1, 5, 5}, "right")

    --SCREEN 1-2
    newShape("shape","triangle1_1_2", {1, 2, 1, 2}, "triangleBottomRightShape", {1, _, "breakable"})
    newShape("autoFan","autoFan1_1_2", {1, 2, 4, 2}, "right")
    newTransition("autoFan1_1_2", "slide", {"orange"}, {{1,2,2,1},{1,2,3,1},{1,2,3,2,"*"},{1,2,4,2},{1,2,4,3},{1,2,3,3,"*"},{1,2,3,4}})
    newShape("door","door1_1_2", {1, 2, 7, 2}, "right")
    newShape("manualFan","manualFan1_1_2", {1, 2, 5, 3}, "down")
    newTransition("manualFan1_1_2", "slide", {"silver"}, {{1,2,5,3},{1,2,6,2},{1,2,6,1}})
    newShape("shape","bar1_1_2", {1, 2, 2, 4}, "bar", {"horz", 3})
    newShape("shape","triangle2_1_2", {1, 2, 6, 4}, "triangleTopLeftShape")
    newTransition("triangle2_1_2", "slide", {"orange"}, {{1,2,6,4},{1,2,6,5}})
    newShape("endPoint","endPoint1_1_2", {1, 2, 7, 4})
    newShape("shape","triangle3_1_2", {1, 2, 1, 5}, "triangleTopLeftShape", {1, 6, "breakable"})

    --SCREEN 2-2
    newShape("item","map", {2, 2, 1, 2})
    newShape("characterChangePoint","characterChangePoint1_2_2", {2, 2, 2, 2})
    newShape("shape","triangle1_2_2", {2, 2, 6, 2}, "triangleBottomLeftShape", {1, _, "hyroll"})
    newShape("door","door1_2_2", {2, 2, 7, 2}, "right")
    newShape("door","door2_2_2", {2, 2, 1, 4}, "left")
    newShape("shape","triangle1_2_2", {2, 2, 6, 4}, "triangleTopLeftShape", {1, _, "hyroll"})

    --SCREEN 3-2
    newShape("shape","triangle1_3_2", {3, 2, 3, 2}, "triangleBottomLeftShape", {1, _, "hyroll"})
    newShape("shape","triangle2_3_2", {3, 2, 6, 2}, "triangleBottomLeftShape", {1, _, "icy"})
    newShape("door","door1_3_2", {3, 2, 1, 4}, "left")
    newShape("shape","triangle3_3_2", {3, 2, 3, 4}, "triangleTopLeftShape", {1, _, "hyroll"})
    newShape("shape","triangle4_3_2", {3, 2, 6, 4}, "triangleTopLeftShape", {1, _, "icy"})

    --GEMS
    newShape("gem","purple", {1, 1, 3, 1}, "purple")
    newShape("gem","purple", {1, 1, 4, 1}, "purple")
    newShape("gem","purple", {1, 1, 2, 2}, "purple")
    newShape("gem","purple", {1, 1, 4, 2}, "purple")
    newShape("gem","purple", {1, 1, 6, 2}, "purple")
    newShape("gem","purple", {1, 1, 5, 4}, "purple")
    newShape("gem","purple", {1, 1, 2, 5}, "purple")
    newShape("gem","purple", {1, 2, 2, 2}, "purple")
    newShape("gem","purple", {1, 2, 2, 4}, "purple")
    newShape("gem","purple", {1, 2, 4, 4}, "purple")
    newShape("gem","purple", {1, 2, 3, 5}, "purple")
    newShape("gem","purple", {1, 2, 5, 5}, "purple")
    newShape("gem","purple", {1, 2, 7, 5}, "purple")
    newShape("gem","purple", {1, 2, 5, 2}, "purple")
    newShape("gem","purple", {1, 2, 7, 2}, "purple")
    newShape("gem","purple", {1, 2, 1, 1}, "purple")
    newShape("gem","purple", {1, 2, 1, 4}, "purple")
    newShape("gem","purple", {1, 2, 7, 1}, "purple")
    newShape("gem","purple", {2, 2, 3, 2}, "purple")
    newShape("gem","purple", {2, 2, 5, 2}, "purple")
    newShape("gem","purple", {2, 2, 1, 4}, "purple")
    newShape("gem","purple", {2, 2, 3, 4}, "purple")
    newShape("gem","purple", {2, 2, 5, 4}, "purple")
    newShape("gem","purple", {2, 2, 7, 2}, "purple")
    newShape("gem","purple", {2, 2, 7, 4}, "purple")
    newShape("gem","purple", {3, 2, 2, 2}, "purple")
    newShape("gem","purple", {3, 2, 2, 4}, "purple")
    newShape("gem","purple", {3, 2, 4, 2}, "purple")
    newShape("gem","purple", {3, 2, 4, 4}, "purple")
    newShape("gem","purple", {3, 2, 6, 3}, "purple")
    newShape("gem","redCoin", {1, 2, 4, 5}, "redCoin")
    newShape("gem","blueCoin", {3, 2, 5, 4}, "blueCoin")
    newShape("gem","purpleCoin", {2, 2, 4, 4}, "purpleCoin")

    --ITEMS
    newShape("item","coins", {1, 2, 5, 4}, 20)
    newShape("item","coins", {2, 2, 6, 3}, 250)
    newShape("item","coins", {3, 2, 1, 4}, 500)
    newShape("item","coins", {1, 2, 4, 1}, 90)
    newShape("item","bomb", {1, 1, 2, 4 })
    newShape("item","small-present", {1, 1, 2, 3}, {})

    newShape("item","small-present", {1, 2, 1, 3}, {})

    newShape("item","big-present", {3, 2, 5, 2}, {})


end
    t.createLevelObjects = createLevelObjects

t.thisScreenHorzValue = 1
t.thisScreenVertValue = 1
t.screenHorzTotal = 3
t.screenVertTotal = 2
t.ballFirstScreenHorzValue = 1
t.ballFirstScreenVertValue = 1
t.ballFirstHorzSquare = 1
t.ballFirstVertSquare = 2
t.ballFirstDirection = "right"
t.initialDirection = t.ballFirstDirection

return t
