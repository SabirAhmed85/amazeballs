local t = {}
local addToShapeArray = require("utils.add-to-shape-array");
local addToTransitionArray = require("utils.add-to-transition-array");

local createLevelObjects = function (mainFunc, shapeArrayParameters)
    local newShape = function(type, name, location, subType, props)
        addToShapeArray(shapeArrayParameters, type, name, location, subType, props);
    end
    local newTransition = function(shapeName, transitionType, props, positionArray)
        addToTransitionArray(mainFunc.allLevelSettings.transitionArrayIndex, shapeArrayParameters, shapeName, transitionType, props, positionArray);
    end

    --SCREEN 1-1
    newShape("shape","triangle1_1_1", {1, 1, 3, 1}, "triangleTopAndBottomShape", {2})
    newShape("endPoint","endPoint1_1_1", {1, 1, 1, 2})
    newShape("shape","triangle2_1_1", {1, 1, 2, 2}, "triangleBottomLeftShape", {1, 2})
    newShape("shape","triangle3_1_1", {1, 1, 6, 2}, "triangleBottomLeftShape")
    newShape("shape","triangle4_1_1", {1, 1, 3, 3}, "triangleTopRightShape")
    newShape("shape","triangle5_1_1", {1, 1, 7, 3}, "triangleBottomLeftShape")
    newShape("shape","triangle6_1_1", {1, 1, 4, 4}, "triangleTopRightShape")
    newShape("autoFan","autoFan1_1_1", {1, 1, 7, 4}, "right")
    newShape("door","door3_1_1", {1, 1, 7, 4}, "right")
    newShape("shape","triangle7_1_1", {1, 1, 1, 5}, "triangleTopRightShape")
    newShape("shape","triangle8_1_1", {1, 1, 5, 5}, "triangleTopRightShape", {1, 10, "breakable"})
    newShape("autoFan","autoFan2_1_1", {1, 1, 6, 5}, "up")
    newTransition("autoFan2_1_1", "slide", {}, {{1,1,6,3},{1,1,6,5}})

    --SCREEN 2-1
    newShape("door","door1_2_1", {2, 1, 1, 1}, "left")
    newShape("autoFan","autoFan1_2_1", {2, 1, 5, 1}, "left")
    newShape("autoFan","autoFan2_2_1", {2, 1, 1, 2}, "down")
    newTransition("autoFan2_2_1", "slide", {}, {{2,1,1,2},{2,1,2,2}})
    newShape("autoFan","autoFan3_2_1", {2, 1, 3, 2}, "right")
    newShape("door","door2_2_1", {2, 1, 7, 2}, "right")
    newShape("shape","triangle1_2_1", {2, 1, 2, 3}, "triangleTopLeftShape", {1, 11, "breakable"})
    newShape("shape","triangle2_2_1", {2, 1, 5, 3}, "triangleTopRightShape")
    newShape("shape","triangle3_2_1", {2, 1, 6, 3}, "triangleBottomLeftShape")
    newShape("shape","triangle4_2_1", {2, 1, 3, 4}, "triangleTopLeftShape")
    newShape("door","door1_2_1", {2, 1, 1, 5}, "left")
    newShape("shape","triangle5_2_1", {2, 1, 1, 5}, "triangleTopRightShape", {1, _, "breakable"})
    newShape("manualFan","manualFan1_2_1", {2, 1, 3, 5}, "left")
    newTransition("manualFan1_2_1", "slide", {}, {{2,1,2,5},{2,1,3,5}})
    newShape("autoFan","autoFan4_2_1", {2, 1, 6, 5}, "up")

    --SCREEN 3-1
    newShape("shape","bar1_3_1", {3, 1, 5, 1}, "bar", {"vert", 3})
    newShape("shape","triangle1_3_1", {3, 1, 5, 1}, "triangleBottomLeftShape", {1, _, "breakable"})
    newShape("shape","triangle2_3_1", {3, 1, 3, 2}, "triangleBottomLeftShape")
    newShape("manualFan","manualFan1_3_1", {3, 1, 2, 3}, "up")
    newShape("autoFan","autoFan1_3_1", {3, 1, 3, 3}, "right")
    newShape("shape","triangle3_3_1", {3, 1, 7, 3}, "triangleTopLeftShape")
    newShape("shape","triangle4_3_1", {3, 1, 1, 4}, "triangleTopRightShape", {1, _, "breakable"})
    newShape("shape","bar2_3_1", {3, 1, 3, 4}, "bar", {"horz", 3})
    newShape("shape","triangle5_3_1", {3, 1, 5, 4}, "triangleTopLeftShape")
    newTransition("triangle5_3_1", "slide", {}, {{3,1,5,3},{3,1,5,4}})
    newShape("door","door1_3_1", {3, 1, 1, 5}, "left")
    newShape("shape","triangle6_3_1", {3, 1, 7, 5}, "triangleBottomLeftShape")

    --GEMS
    newShape("gem","purple", {1, 1, 2, 1}, "purple")
    newShape("gem","purple", {1, 1, 3, 2}, "purple")
    newShape("gem","purple", {1, 1, 4, 3}, "purple")
    newShape("gem","purple", {1, 1, 4, 5}, "purple")
    newShape("gem","purple", {1, 1, 1, 3}, "purple")
    newShape("gem","purple", {1, 1, 7, 5}, "purple")
    newShape("gem","purple", {2, 1, 2, 4}, "purple")
    newShape("gem","purple", {2, 1, 3, 3}, "purple")
    newShape("gem","purple", {2, 1, 4, 2}, "purple")
    newShape("gem","purple", {2, 1, 6, 2}, "purple")
    newShape("gem","purple", {2, 1, 1, 1}, "purple")
    newShape("gem","purple", {2, 1, 3, 1}, "purple")
    newShape("gem","purple", {2, 1, 6, 4}, "purple")
    newShape("gem","purple", {2, 1, 4, 5}, "purple")
    newShape("gem","purple", {2, 1, 7, 5}, "purple")
    newShape("gem","purple", {3, 1, 2, 2}, "purple")
    newShape("gem","purple", {3, 1, 4, 3}, "purple")
    newShape("gem","purple", {3, 1, 6, 3}, "purple")
    newShape("gem","purple", {3, 1, 7, 1}, "purple")
    newShape("gem","purple", {3, 1, 5, 5}, "purple")
    newShape("gem","purple", {3, 1, 3, 5}, "purple")
    newShape("gem","purple", {3, 1, 1, 5}, "purple")
    newShape("gem","purple", {3, 1, 1, 1}, "purple")
    newShape("gem","purple", {3, 1, 3, 1}, "purple")
    newShape("gem","purple", {3, 1, 5, 2}, "purple")
    newShape("gem","redCoin", {2, 1, 1, 3}, "redCoin")
    newShape("gem","blueCoin", {3, 1, 2, 4}, "blueCoin")
    newShape("gem","purpleCoin", {1, 1, 2, 5}, "purpleCoin")

    --ITEMS
    newShape("item","coins", {1, 1, 4, 2}, 80)
    newShape("item","coins", {2, 1, 7, 2}, 12)
    newShape("item","coins", {3, 1, 7, 2}, 75)
    newShape("item","small-present", {1, 1, 3, 5}, {})

    newShape("item","small-present", {3, 1, 4, 5}, {})

    newShape("item","big-present", {3, 1, 1, 3}, {})


end
    t.createLevelObjects = createLevelObjects

t.thisScreenHorzValue = 1
t.thisScreenVertValue = 1
t.screenHorzTotal = 3
t.screenVertTotal = 1
t.ballFirstScreenHorzValue = 1
t.ballFirstScreenVertValue = 1
t.ballFirstHorzSquare = 1
t.ballFirstVertSquare = 1
t.ballFirstDirection = "right"
t.initialDirection = t.ballFirstDirection

return t
