local t = {}
local addToShapeArray = require("utils.add-to-shape-array");
local addToTransitionArray = require("utils.add-to-transition-array");

local createLevelObjects = function (mainFunc, shapeArrayParameters)
    local newShape = function(type, name, location, subType, props)
        addToShapeArray(shapeArrayParameters, type, name, location, subType, props);
    end
    local newTransition = function(shapeName, transitionType, props, positionArray)
        addToTransitionArray(mainFunc.allLevelSettings.transitionArrayIndex, shapeArrayParameters, shapeName, transitionType, props, positionArray);
    end
    
    --SCREEN 1-1
    newShape("autoFan","autoFan1_1_1", {1, 1, 1, 1}, "right")
    newShape("shape","triangle1_1_1", {1, 1, 4, 1}, "triangleTopAndBottomShape", {2})
    newTransition("triangle1_1_1", "slide", {"green"}, {{1,1,3,1},{1,1,4,1}})
    newShape("autoFan","autoFan2_1_1", {1, 1, 7, 1}, "left")
    newShape("shape","triangle2_1_1", {1, 1, 6, 2}, "triangleBottomLeftShape", {1, 1})
    newShape("endPoint","endPoint1_1_1", {1, 1, 2, 3})
    newShape("autoFan","autoFan3_1_1", {1, 1, 4, 4}, "right")
    newShape("door","door1_1_1", {1, 1, 7, 4}, "right")
    newShape("shape","triangle3_1_1", {1, 1, 1, 5}, "triangleTopAndBottomShape", {1})
    newShape("shape","triangle4_1_1", {1, 1, 6, 5}, "triangleTopAndBottomShape", {1})

    --SCREEN 2-1
    newShape("door","door1_2_1", {2, 1, 1, 1}, "left")
    newShape("shape","triangle1_2_1", {2, 1, 5, 1}, "triangleBottomRightShape", {1, 8})
    newShape("autoFan","autoFan1_2_1", {2, 1, 6, 1}, "right")
    newTransition("autoFan1_2_1", "slide", {"orange"}, {{2,1,6,1},{2,1,7,1}})
    newShape("door","door2_2_1", {2, 1, 1, 2}, "left")
    newShape("shape","triangle2_2_1", {2, 1, 5, 2}, "triangleTopRightShape", {1, 1})
    newShape("autoFan","autoFan2_2_1", {2, 1, 6, 2}, "right")
    newTransition("autoFan2_2_1", "slide", {"orange"}, {{2,1,6,2},{2,1,7,2}})
    newShape("door","door3_2_1", {2, 1, 1, 3}, "left")
    newShape("shape","triangle3_2_1", {2, 1, 3, 3}, "triangleTopLeftShape")
    newShape("shape","triangle4_2_1", {2, 1, 1, 5}, "triangleBottomRightShape")
    newTransition("triangle4_2_1", "slide", {"green"}, {{2,1,1,5},{2,1,2,4},{2,1,3,4}})
    newShape("autoFan","autoFan3_2_1", {2, 1, 4, 5}, "right")
    newShape("item","map", {2, 1, 6, 5})
    newShape("autoFan","autoFan4_2_1", {2, 1, 7, 5}, "up")

    --GEMS
    newShape("gem","purple", {1, 1, 4, 2}, "purple")
    newShape("gem","purple", {1, 1, 5, 5}, "purple")
    newShape("gem","purple", {2, 1, 1, 1}, "purple")
    newShape("gem","purple", {2, 1, 3, 1}, "purple")
    newShape("gem","purple", {2, 1, 4, 2}, "purple")
    newShape("gem","purple", {2, 1, 4, 4}, "purple")
    newShape("gem","purple", {2, 1, 5, 5}, "purple")
    newShape("gem","purple", {2, 1, 7, 4}, "purple")
    newShape("gem","purple", {2, 1, 3, 2}, "purple")
    newShape("gem","purple", {2, 1, 4, 3}, "purple")
    newShape("gem","purple", {2, 1, 1, 4}, "purple")
    newShape("gem","purple", {1, 1, 7, 3}, "purple")
    newShape("gem","purple", {1, 1, 5, 3}, "purple")
    newShape("gem","purple", {1, 1, 7, 3}, "purple")
    newShape("gem","purple", {1, 1, 3, 3}, "purple")
    newShape("gem","redCoin", {1, 1, 6, 4}, "redCoin")
    newShape("gem","blueCoin", {2, 1, 2, 2}, "blueCoin")
    newShape("gem","purpleCoin", {2, 1, 1, 3}, "purpleCoin")

    --ITEMS
    newShape("item","coins", {2, 1, 5, 4}, 90)
    newShape("item","coins", {1, 1, 6, 1}, 50)
    newShape("item","coins", {2, 1, 1, 2}, 150)
    newShape("item","coins", {1, 1, 5, 2}, 150)
    newShape("item","small-present", {2, 1, 7, 3}, {})


end
    t.createLevelObjects = createLevelObjects

t.thisScreenHorzValue = 1
t.thisScreenVertValue = 1
t.screenHorzTotal = 2
t.screenVertTotal = 1
t.ballFirstScreenHorzValue = 2
t.ballFirstScreenVertValue = 1
t.ballFirstHorzSquare = 4
t.ballFirstVertSquare = 1
t.ballFirstDirection = "down"
t.initialDirection = t.ballFirstDirection

return t